...
export default function DanhSachKyThi() {
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            try {
                ...
                const examsData = Array.isArray(examsRes.data?.exams) ? examsRes.data.exams : []
                ...
                if (!mounted) return
                setError(err?.response?.data?.message || "Không thể tải dữ liệu kỳ thi")
            } finally {
    ...
    const examCountByClass = useMemo(() => {
        ...
        exams.forEach((exam) => {
            if (!exam.lop_id) return
            map.set(exam.lop_id, (map.get(exam.lop_id) || 0) + 1)
        })
        return map
    }, [exams])
        ...
        <DashboardLayout role="SINH VIÊN">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                {!loading && !error && filteredClasses.map((lop) => (
                    ...
                    <div
                        key={lop.id}
                        onClick={() => navigate(`/sinhvien/ky-thi/${lop.id}`)}
                    ...
                    >
                        ...
                        <p className="text-sm text-slate-500 mt-2">
                            {examCountByClass.get(lop.id) || 0} kỳ thi
                        </p>
