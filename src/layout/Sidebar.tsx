...
const menuByRole = {
  ...
  "ADMIN": [
    { icon: LayoutDashboard, label: "Trang chủ", href: "/admin/dashboard" },
    { icon: Users, label: "Quản lý tài khoản", href: "/admin/accounts" },
    { icon: BookOpen, label: "Quản lý môn học", href: "/admin/subjects" },
    { icon: Settings, label: "Quản lý hệ thống", href: "/admin/system" },
  ],
}
...
const Sidebar = memo(function Sidebar({ role = "SINH VIÊN" }: SidebarProps) {
  ...
  const navItems: NavItem[] =
    menuByRole[role as keyof typeof menuByRole] || menuByRole["SINH VIÊN"]
    ...
    <aside className="w-64 bg-gradient-to-b from-slate-900 via-slate-800 to-indigo-900 text-white flex flex-col shadow-lg">
      ...
      <nav className="flex-1 p-4 space-y-1 overflow-y-auto">
        ...
        {navItems.map((item) => {
          ...
          // active nếu pathname bắt đầu bằng href
